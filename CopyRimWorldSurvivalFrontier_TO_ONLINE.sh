...
DESTINATION_PATH_RAW=${SOURCE_PATH_RAW}

# ================= MOD SOURCE (development files) =================
SOURCE="${SOURCE_PATH_RAW}/${GAMENAME}/${MODNAME}/"
DESTINATION="${DESTINATION_PATH_RAW}/${GAMENAME}/Online/${MODNAME}/"
mkdir -p "${DESTINATION}"

# ================= PARAMETERS =================
EXCLUDES=(--exclude=bin/ --exclude=obj/ --exclude=libs --exclude=.git/ --exclude=.idea/ --exclude=.vscode/)
...
if rsync "${PARAMETERS[@]}" "${SOURCE}" "${DESTINATION}"; then
    ...
    DIFF=$(rsync "${PARAMETERS_VERIFY[@]}" "${SOURCE}" "${DESTINATION}" 2>&1)
